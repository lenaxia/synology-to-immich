...
Env vars:
  ...
  DRY_RUN         If "true", no DB writes or name assignments (default: false)
...
def compute_iou(syno_bbox, imm_face):
    """Intersection-over-Union of a Synology bbox (normalized) vs Immich face (pixels)."""
...
def get_named_persons(syno_conn, target_name=None):
    syno_ids = list(cfg.syno_to_immich.keys())
...
def assign_person_name(api_key, immich_url, person_id, name):
    """PUT /api/people/{id} to set the name. Returns (ok, old_name)."""
    try:
        resp = requests.get(
